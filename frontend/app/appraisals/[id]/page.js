...
export default function AppraisalDetail({ params }) {
    ...
    return (
      <DashboardLayout>
        <div className="p-4">
          ...
          <button className="btn-primary" onClick={() => router.push("/appraisals")}>
            Back to Appraisals
          </button>
        </div>
      </DashboardLayout>
    ...
    return (
      <DashboardLayout>
        <div className="p-4">
          ...
          <p className="mb-4">You do not have permission to view this appraisal.</p>
          <button className="btn-primary" onClick={() => router.push("/appraisals")}>
            Back to Appraisals
          </button>
        </div>
  ...
  return (
    <DashboardLayout>
      <div className="p-4">
        <h1 className="text-2xl font-bold mb-6">Appraisal for {appraisal?.targetUser?.name}</h1>
        ...
        <div className="card mb-4">
          ...
          <div className="mb-4">
            ...
            <p className="font-medium">{new Date(appraisal.createdAt).toLocaleDateString()}</p>
          </div>

          {appraisal.isSelfAppraisal && (
            <div className="mb-4 inline-block bg-blue-100 text-blue-800 px-2 py-1 rounded text-sm">Self Appraisal</div>
          )}
        </div>
